...
var
    ...
    debug = require('debug')('express-webapp-assets:assets'),
    DEBUG = debug.enabled;
...
function Assets(config) {
    ...
    this.config.src = path.resolve(process.cwd(), this.config.src);
    this.config.dst = path.resolve(process.cwd(), this.config.dst);
    this.config.includes = _.compact(_.uniq(this.config.includes)).map(function (inc) {
    ...
    this.config.helpers.js = function (name) {
        ...
        return self._getBundleAssets(name).map(function (name) {
            return '<script src="' + name + '"></script>';
        }).join('\n');
    };
...
Assets.prototype._getBundleAssets = function (name) {
    var src = path.join(this.config.src, this._getUncompressedFile(name) + '.bundle');
    var data = fs.readFileSync(src, 'utf8');
    var self = this;
    return data.split('\n').filter(function (line) {
        return line.length > 0 && line.charAt(0) !== '#';
    }).map(function (name) {
        return self._getUncompressedFile(name);
    });
};
...
Assets.prototype.resolve = function (name, callback) {
    ...
    if (fs.existsSync(src)) {
        ...
        if (stat.isDirectory()) {
            ...
            //src = path.join(this.config.src, name);
        }
    }
    ...
    this.copy(assetSrc, assetDst, function (err) {
        ...
        self.bundle(assetSrc, assetDst, function (err) {
            ...
            self.filter(assetSrc, assetDst, function (err) {
                ...
                self.include(self.config.includes, name, dst, function (includeErr) {
                    if (!includeErr) {
                        return callback(null, dst);
                    }
                    // NOTE: include fail is not error. it's just a fallback.
                    // err.errors.push(includeErr);
                    callback(err);
                });
            });
        });
